...
function ossn_embed_create_embed_object($url, $guid, $videowidth=0) {
	...
	if (strpos($url, 'youtube.com') != false) {
		...
		return ossn_embed_cinnamon_handler($url, $guid, $videowidth);
 ...
 * @return string css style, video div, and flash <object>
 */
function ossn_embed_bitchute_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = ossn_embed_bitchute_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}
	...
	$embed_object .= ossn_embed_add_object('bitchute', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function ossn_embed_bitchute_parse_url($url) {
	// separate parsing embed url
	if (strpos($url, 'object') != false) {
		return ossn_embed_bitchute_parse_embed($url);
	}
 ...
 * @return string css style, video div, and flash <object>
 */
function ossn_embed_cinnamon_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = ossn_embed_cinnamon_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}
	...
	$embed_object .= ossn_embed_add_object('cinnamon', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function ossn_embed_cinnamon_parse_url($url) {
	// separate parsing embed url
	if (strpos($url, 'object') != false) {
		return ossn_embed_cinnamon_parse_embed($url);
	}                     
	if (preg_match('/(https:\/\/)(www\.)?(cinnamon.video\/watch[?]v=)(.*)/', $url, $matches)) {
			// that's the "embed" link suggested by cinnamon
			return $matches[4];
	}
